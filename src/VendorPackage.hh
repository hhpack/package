...
final class VendorPackage implements FromItems<Pair<string, string>>
{
    ...
    public function __construct(
        Traversable<VendorNamespace> $namespaces
    )
    {
    ...
    {
        $results = $this->namespaces->map(($namespace) ==> $namespace->sources($matcher));
    ...
    {
        $results = $this->namespaces->map(($namespace) ==> $namespace->classes($matcher));

        return ResourceStream::fromItems($results)
            ->reduce(new ResourceStream(), ($a, $b) ==> $a->concat($b));
    }

    public static function fromItems(Traversable<Pair<string, string>> $items) : this
    {
        $results = ImmVector::fromItems($items)->map(($item) ==> VendorNamespace::of($item));
